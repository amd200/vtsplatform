...
const PaymentDialog = ({ isOpen, onOpenChange, total, invoiceId, cartCourses }: { isOpen: boolean; onOpenChange: (open: boolean) => void; total: number; invoiceId?: string; cartCourses: CartItem[] | null | undefined }) => {
  ...
  const [CoursePaymentWithFawry] = useCoursePaymentWithFawryMutation();
  ...
  const handleConfirmPayment = async () => {
    try {
      ...
      if (paymentMethod == "fawry" && invoiceId) {
        const res = await CoursePaymentWithFawry(invoiceId);
        console.log(res);
      }
    } catch (error) {}
...
function Payment({ invoiceId }: { invoiceId?: string }) {
